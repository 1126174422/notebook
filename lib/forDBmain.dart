...
class _MyHomePageState extends State<MyHomePage> {
  ...
  getLocalDatas() async {
    List<dynamic> values = await db.getAllNotes();
    //prefs.
    setState(() {
      if (values.length > 0) {
        tags.clear();
                                ...
                                onChanged: (select) {
                                  setState(() {
                                    if (select) {
                                      ...
                                      selected.remove(tags[index].id);
                                    }
                                  });
                                },
                                value: selected.contains(tags[index].id),
                              ),
                            )
                          ],
                        ),
